...
# Scenario 1 - Task 4 : Repair the Application Load Balancer
#
# Checks performed (all must pass):
#   1. pn-alb-sg allows TCP 80 from 0.0.0.0/0
#   2. pn-web-tg health check path is /health
#   3. The HTTP:80 listener forwards to pn-web-tg
#   4. pn-web-01 is healthy in pn-web-tg
#   5. The ALB DNS name returns the PAYNOTIFY page with HTTP 200
#
...
try {
    ...
    if ($did) {
        $lb = Get-ELB2LoadBalancer -Name "pn-alb-$did" -ErrorAction SilentlyContinue
        $tg = Get-ELB2TargetGroup -Name "pn-web-tg-$did" -ErrorAction SilentlyContinue
    } else {
        $lb = Get-ELB2LoadBalancer | Where-Object { $_.LoadBalancerName -like 'pn-alb-*' } | Select-Object -First 1
        $tg = Get-ELB2TargetGroup | Where-Object { $_.TargetGroupName -like 'pn-web-tg-*' } | Select-Object -First 1
    }
    $albSg = Get-TaggedSG 'pn-alb-sg'
    $web   = Get-TaggedInstance 'pn-web-01'
    if (-not $lb -or -not $tg -or -not $albSg) { Add-Check $false 'The load balancer pn-alb, target group pn-web-tg or pn-alb-sg was not found. Contact your instructor.' }
    else {
        Add-Check (Test-SGRule $albSg.IpPermissions 80 $null '8.8.8.8') 'Task 4 - pn-alb-sg does not allow inbound HTTP (TCP 80) from 0.0.0.0/0.'
        Add-Check ($tg.HealthCheckPath -eq '/health') "Task 4 - pn-web-tg health check path is '$($tg.HealthCheckPath)', it must be /health."

        $l80 = Get-ELB2Listener -LoadBalancerArn $lb.LoadBalancerArn | Where-Object { $_.Port -eq 80 } | Select-Object -First 1
        $fwd = $false
        if ($l80) {
            foreach ($a in $l80.DefaultActions) {
                if ("$($a.Type)" -eq 'forward') {
                    if ($a.TargetGroupArn -eq $tg.TargetGroupArn) { $fwd = $true }
                    if ($a.ForwardConfig -and ($a.ForwardConfig.TargetGroups | Where-Object { $_.TargetGroupArn -eq $tg.TargetGroupArn })) { $fwd = $true }
                }
            }
        }
        Add-Check $fwd 'Task 4 - the HTTP:80 listener does not forward to pn-web-tg. Change its default action from the fixed 503 response to Forward.'

        $th = Get-ELB2TargetHealth -TargetGroupArn $tg.TargetGroupArn
        $healthy = @($th | Where-Object { $web -and $_.Target.Id -eq $web.InstanceId -and "$($_.TargetHealth.State)" -eq 'healthy' }).Count -gt 0
        $reason = ($th | Select-Object -First 1).TargetHealth.Description
        Add-Check $healthy "Task 4 - pn-web-01 is not healthy in pn-web-tg ($reason)."

        $webOk = $false
        try {
            $resp = Invoke-WebRequest -Uri "http://$($lb.DNSName)/" -UseBasicParsing -TimeoutSec 10
            $webOk = ($resp.StatusCode -eq 200 -and $resp.Content -match 'PAYNOTIFY')
        } catch { $webOk = $false }
        Add-Check $webOk 'Task 4 - the load balancer DNS name does not return the PAYNOTIFY page with HTTP 200 yet.'
    }
