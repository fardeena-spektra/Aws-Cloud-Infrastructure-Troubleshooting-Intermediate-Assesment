...
# Scenario 2 - Task 1 : Restore Outbound Internet for the Private App Instance
#
# Checks performed (all must pass):
#   1. pn-private-rt sends 0.0.0.0/0 to a NAT gateway (not the internet gateway)
#   2. The NAT gateway is available and sits in a public subnet
#   3. pn-app-01 has no public IP address
#   4. pn-app-01 is Online in Systems Manager
#
...
try {
    ...
    $sn  = Get-TaggedSubnet 'pn-app-subnet'
    if (-not $app -or -not $sn) { Add-Check $false 'pn-app-01 or pn-app-subnet was not found. Contact your instructor.' }
    else {
        $rt = Get-EC2RouteTable -Filter @{ Name = 'association.subnet-id'; Values = @($sn.SubnetId) } | Select-Object -First 1
        $def = $rt.Routes | Where-Object { $_.DestinationCidrBlock -eq '0.0.0.0/0' } | Select-Object -First 1
        $natId = if ($def) { $def.NatGatewayId } else { $null }
        Add-Check ([bool]$natId) 'Task 1 - the route table of pn-app-subnet does not send 0.0.0.0/0 to a NAT gateway.'

        $natOk = $false
        if ($natId) {
            $nat = Get-EC2NatGateway -NatGatewayId $natId
            if ("$($nat.State)" -eq 'available') {
                $nrt = Get-EC2RouteTable -Filter @{ Name = 'association.subnet-id'; Values = @($nat.SubnetId) } | Select-Object -First 1
                if (-not $nrt) { $nrt = Get-EC2RouteTable -Filter @(@{ Name = 'vpc-id'; Values = @($sn.VpcId) }, @{ Name = 'association.main'; Values = @('true') }) | Select-Object -First 1 }
                $natOk = [bool]($nrt.Routes | Where-Object { $_.DestinationCidrBlock -eq '0.0.0.0/0' -and $_.GatewayId -like 'igw-*' })
            }
        }
        Add-Check $natOk 'Task 1 - the NAT gateway used by pn-app-subnet is not available in a public subnet.'
        Add-Check (-not $app.PublicIpAddress) 'Task 1 - pn-app-01 must stay private. Remove any public or Elastic IP address from it.'
        Add-Check (Test-SSMOnline $app.InstanceId) 'Task 1 - pn-app-01 is not Online in Systems Manager yet. Reboot it, wait 3 to 5 minutes, then validate again.'
    }
