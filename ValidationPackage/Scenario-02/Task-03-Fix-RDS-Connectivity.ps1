...
# Scenario 2 - Task 3 : Fix EC2-to-RDS Connectivity
#
# Checks performed (all must pass):
#   1. pn-orders-db is available
#   2. pn-db-sg allows TCP 3306 from pn-app-sg
#   3. The stale TCP 3306 rule from pn-web-sg is removed
#   4. pn-app-sg allows outbound TCP 3306 to the database
#
...
try {
    ...
    $dbId = if ($did) { "pn-orders-db-$did" } else { $null }
    $db = if ($dbId) { Get-RDSDBInstance -DBInstanceIdentifier $dbId -ErrorAction SilentlyContinue } else { Get-RDSDBInstance | Where-Object { $_.DBInstanceIdentifier -like 'pn-orders-db-*' } | Select-Object -First 1 }
    $appSg = Get-TaggedSG 'pn-app-sg'
    $dbSg  = Get-TaggedSG 'pn-db-sg'
    $webSg = Get-TaggedSG 'pn-web-sg'
    $app   = Get-TaggedInstance 'pn-app-01'
    if (-not $db -or -not $appSg -or -not $dbSg) { Add-Check $false 'pn-orders-db, pn-app-sg or pn-db-sg was not found. Contact your instructor.' }
    else {
        Add-Check ($db.DBInstanceStatus -eq 'available') "Task 3 - pn-orders-db is '$($db.DBInstanceStatus)'. Wait until it is available."
        $appIp = if ($app) { $app.PrivateIpAddress } else { $null }
        Add-Check (Test-SGRule $dbSg.IpPermissions 3306 $appSg.GroupId $appIp) 'Task 3 - pn-db-sg has no inbound rule for TCP 3306 from pn-app-sg.'
        $stale = $webSg -and (Test-SGRule $dbSg.IpPermissions 3306 $webSg.GroupId $null)
        Add-Check (-not $stale) 'Task 3 - pn-db-sg still trusts pn-web-sg on TCP 3306. The web tier must not reach the database, remove that rule.'
        $egOk = Test-SGRule $appSg.IpPermissionsEgress 3306 $dbSg.GroupId $null
        if (-not $egOk) {
            foreach ($s in @('pn-db-subnet-a', 'pn-db-subnet-b')) {
                $sn = Get-TaggedSubnet $s
                if ($sn -and (Test-SGRule $appSg.IpPermissionsEgress 3306 $null (Get-SampleIP $sn.CidrBlock))) { $egOk = $true }
            }
        }
        Add-Check $egOk 'Task 3 - pn-app-sg has no outbound rule for TCP 3306 to pn-db-sg.'
    }
